...
module.exports = function (config, context) {
  ...
  var truncateParamValue = function (type, paramValue) {
    ...
    if (paramValue && paramLengthLimit < paramValue.length) {
      console.warn(util.format('NewrelicLib (truncateParamValue): Newrelic parameter %s with value %s has been truncated.', type, paramValue))
  ...
  var recordDeploymentToNewrelic = function (sns, user, revision) {
    ...
    if (!applicationConfig) {
      context.fail(util.format('NewrelicLib (recordDeploymentToNewrelic): No application config found for applicationName: "%s" in config: "%s"', snsMessage.applicationName, config))
      return
    }

    if (!template) {
      console.log(util.format('NewrelicLib (recordDeploymentToNewrelic): No template setup for status: "%s", could be ok.', snsMessage.status))
      return
    }
    ...
    return new Promise(function (resolve, reject) {
      request({
        ...
        if (!S(response.statusCode).startsWith(20)) {
          console.warn(util.format('NewrelicLib (recordDeploymentToNewrelic): Newrelic api returned http status code of: %s, %s', response.statusCode, body))
          reject(response)
        }
        try {
          console.log('NewrelicLib (recordDeploymentToNewrelic): Newrelic api reponse', body)
          return resolve(body)
        } catch (e) {
          return reject(e)
        }
