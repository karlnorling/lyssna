...
module.exports = function (config) {
  ...
  var triggerPagerDutyAlert = function (sns, user, revison) {
    ...
    if (!template) {
      console.warn(util.format('PagerDutyLib: No template for sns status "%s"', snsStatus))
      return
    }
    ...
    return new Promise(function (resolve, reject) {
      request({
        ...
        if (!S(response.statusCode).startsWith(20)) {
          console.warn(util.format('PagerDutyLib: Newrelic api returned http status code of: %s, %s', response.statusCode, body))
          reject(response)
        }
        try {
