...
module.exports = function (config, context) {
  ...
  var triggerPagerDutyAlert = function (sns, user, revison) {
    ...
    if (!applicationConfig) {
      console.warn(util.format('PagerDutyLib (triggerPagerDutyAlert): No application config found for applicationName: "%s" in config: "%s"', snsMessage.applicationName, config))
      return
    }
    ...
    return new Promise(function (resolve, reject) {
      request({
        ...
        try {
          console.log('PagerDutyLib: PagerDuty api reponse', body)
          return resolve(body)
        } catch (e) {
          return reject(e)
        }
