...
module.exports = function (config) {
  ...
  var getStatusData = function (applicationConfig, status) {
    ...
    if (!applicationTemplate) {
      console.warn(util.format('SlackLib: No application template for status: %s', status))
    }
  ...
  var composeMessage = function (snsMessage, revision, metaTags) {
    ...
    var userMessage = function (metaTags) {
      ...
      console.log(util.format('SlackLib: No user associated with deployment: %s', deploymentId))
      return
    }
    ...
    if (!template) {
      console.log(util.format('SlackLib: No template exists for status: %s', snsStatus))
    }
   ...
   * @param {Object} snsMessage
   */
  var postMessage = function (snsMessage, revision, metaTags) {
    var cm = composeMessage(snsMessage, revision, metaTags)
    var channels = getChannels(snsMessage)
    var applicationConfig = getApplicationConfig(snsMessage)
    var statusData = getStatusData(applicationConfig, snsMessage.status.toLowerCase())
    var promises = []
      ...
      promises.push(new Promise(function (resolve, reject) {
        request({
          ...
          if (!S(response.statusCode).startsWith(20)) {
            console.warn(util.format('SlackLib: Slack api returned http status code of: %s, %s', response.statusCode, body))
            reject(response)
          }
          try {
            console.log('SlackLib: Slack api reponse', body)
            return resolve(body)
          } catch (e) {
            return reject(e)
          }
