...
module.exports = function (hipchatConfig) {
  ...
  var composeMessage = function (snsMessage, revision, metaTags) {
    ...
    var userMessage = function (metaTags) {
      ...
      console.log(util.format('HipchatLib: No user associated with deployment: %s', deploymentId))
      return
    }
    ...
    if (!template) {
      console.log(util.format('HipchatLib: No template exists for status: %s', snsStatus))
    }
